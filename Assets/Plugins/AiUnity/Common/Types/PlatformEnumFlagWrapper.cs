...
namespace AiUnity.Common.Types
{
    ...
    public class PlatformEnumFlagWrapper<TEnum> where TEnum : struct, IComparable, IFormattable, IConvertible
    {
        ...
        {
            //string eee = UnityEditor.BuildTargetGroup.WSA.ToString();
            //UnityEditor.BuildTargetGroup qqq = UnityEditor.BuildTargetGroup.WSA;
            //qqq.ToString();

            //var xxx = (UnityEditor.BuildTargetGroup[])Enum.GetValues(typeof(UnityEditor.BuildTargetGroup));
            //var yyy = xxx.Select(e => e.ToString()).ToList();

            //var ttt = typeof(TEnum).ToString();
            //var aaa = EnumUtility.GetValues<TEnum>().Select(e => e.ToString()).ToList();
            EnumValues = EnumUtility.GetValues<TEnum>().Where(e => !this.legacyPlatforms.Any(p => e.ToString().Contains(p))).Distinct().ToList();
            //EnumValues = EnumUtility.GetValues<TEnum>().Distinct().ToList();
            //var bbb = EnumValues.Select(e => e.ToString()).ToList();

            //var ccc = EnumUtility.GetValues<UnityEditor.BuildTarget>().Select(e => e.ToString()).ToList();
